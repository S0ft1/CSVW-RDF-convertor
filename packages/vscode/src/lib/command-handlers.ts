 ...
 */
export async function scanAndLoadAdditionalInputs(conversion: ConversionItem): Promise<void> {
	if (!conversion.folderPath) {
...
export async function openFieldsForConversion(conversion: ConversionItem): Promise<void> {
	...
	if (conversion.inputFilePath) {
		...
		const inputEditor = await vscode.window.showTextDocument(inputDocument, { viewColumn: vscode.ViewColumn.Two,
			preserveFocus: true,
			preview: false});
	...
	if (conversion.rdfInputFilePath) {
		try {
			const rdfInputDocument = await vscode.workspace.openTextDocument(conversion.rdfInputFilePath);
 ...
 * @param conversion - The conversion item
 */
async function openExistingOutputFiles(conversion: ConversionItem): Promise<void> {
	if (!conversion.folderPath) {
		return;
	}

	const outputsDir = vscode.Uri.joinPath(vscode.Uri.file(conversion.folderPath), 'outputs');
