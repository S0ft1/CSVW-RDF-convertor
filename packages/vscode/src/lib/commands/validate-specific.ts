...
import { Csvw2RdfOptions, Issue, validateCsvwFromDescriptor } from '@csvw-rdf-convertor/core';
...
async function validateDocument(conversion: ConversionItem, provider: CSVWActionsProvider) {
	...
	const inputsDir = path.join(conversion.folderPath, 'inputs');
		const csvw2RdfOptions: Csvw2RdfOptions = getCSVWOptions({minimal: false, templateIris: false},inputsDir);
	...
	try {
		...
		if (errorMessages.length === 0) {
			vscode.window.showInformationMessage(`✅ Validation complete for "${conversion.name}". Descriptor is valid!`);
		}
		else {
