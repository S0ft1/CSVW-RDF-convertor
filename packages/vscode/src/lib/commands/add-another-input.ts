 ...
 */
async function findNextInputFileName(
  inputsDir: vscode.Uri,
): Promise<{ fileName: string; filePath: vscode.Uri }> {
  try {
  ...
  const FailedInputFileName = `inputX.csv`;
  return {
 ...
 */
async function createAndOpenInputFile(
 ...
 */
async function updateDescriptorFile(
  ...
  await vscode.workspace.fs.writeFile(
    descriptorUri,
    encoderDesc.encode(updatedDescriptorContent),
  );
 ...
 */
export function registerAddAnotherInput(
  ...
  return vscode.commands.registerCommand(
    ...
    async (conversionId: string) => {
      ...
      try {
        ...
        const { fileName: inputFileName, filePath: inputFilePath } =
          await findNextInputFileName(inputsDir);
        ...
        if (conversion.descriptorFilePath) {
          try {
            ...
            vscode.window.showInformationMessage(
              `✅ Added new input file: ${inputFileName} and updated descriptor for "${conversion.name}"`,
            );
          } catch (parseError) {
            vscode.window.showWarningMessage(
              `⚠️ Added input file but failed to update descriptor: ${parseError}`,
            );
            vscode.window.showInformationMessage(
              `✅ Added new input file: ${inputFileName} for "${conversion.name}"`,
            );
          }
        } else {
          vscode.window.showInformationMessage(
