 ...
 */
export async function readDescriptorContent(conversion: ConversionItem): Promise<string> {
 ...
 */
export function updateConversionState(conversion: ConversionItem, outputFilePaths: string[]): void {
...
export async function handleConversionError(
	...
	try {
		...
		vscode.window.showErrorMessage(`❌ ${conversionDirection} conversion failed: ${error instanceof Error ? error.message : String(error)}`);
	}
}
...
export function ensureConversionPaths(
	conversion: ConversionItem,
	descriptorFileName: string = 'descriptor.jsonld',
	inputFileName: string = 'csvInput.csv',
	rdfInputFileName: string = 'rdfInput.ttl'
): void {
	if (!conversion.descriptorFilePath) {
 ...
 */
export function validateConversionExists(conversion: ConversionItem | undefined): conversion is ConversionItem {
	if (!conversion) {
