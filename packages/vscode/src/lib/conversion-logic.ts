...
export async function convertRDF2CSVW(descriptorText: string, inputPath: string, conversion: ConversionItem): Promise<string[]> {
	...
	let rdfStream = await createRDFStreamInput(inputPath, inputsDir);
	...
	for await ([descriptor, table, row] of stream) {
		if (stringifiers[table.name] === undefined) {
			...
			const descriptorOptions = getAdditionalDescriptorOptions(descriptor, table);
	...
	if (latestDescriptor && !hasDescriptor) {
		...
		if (conversion.descriptorFilePath) {
			try {
				...
				const descriptorDocument = await vscode.workspace.openTextDocument(descriptorUri);
				// Edit the document to make it dirty, then save it to trigger the listener
				const edit = new vscode.WorkspaceEdit();
				edit.replace(descriptorUri, new vscode.Range(0, 0, descriptorDocument.lineCount, 0), descriptorText);
...
export function getCSVWOptions(optionsFromVS: MiniOptions, inputsDirPath: string): Csvw2RdfOptions {
...
function getRDFOptions(inputsDirPath: string, descriptorText: string): Rdf2CsvOptions {
	return {
		baseIri: inputsDirPath,
...
function getAdditionalDescriptorOptions(descriptor: DescriptorWrapper, table: CsvwTable) {
