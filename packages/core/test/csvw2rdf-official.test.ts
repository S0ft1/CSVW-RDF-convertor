    ...
    manifest.entries.forEach(entry => {
        ...
        test(entry.name, async () => {
            try {
                ...
                const options: Csvw2RdfOptions = { resolveJsonldFn: async (url: string, base: string) => {
                ...
                const descriptor = await readFile(resolveMetadataLocation(entry.action, entry?.option?.metadata, entry?.httpLink), 'utf-8')
                    .then(metadata => normalizeDescriptor(metadata, actualStore, options));
                ...
                switch (entry.type) {
                    ...
                    case EntryType.TestWithWarnings: {
                        ...
                        expect((await actualStore.get({})).items).toBeRdfIsomorphic((await expectedStore.get({})).items);
...
function resolveMetadataLocation(actionPath: string, metadataPath?: string, httpLink?: string): string {
    ...
    if (existsSync(resolvedMetadataPath)) return resolvedMetadataPath;
    //     csv-metadata.json
    resolvedMetadataPath = testDir + action.replace(/[^/]*\.csv$/, 'csv-metadata.json');
    if (existsSync(resolvedMetadataPath)) return resolvedMetadataPath;
