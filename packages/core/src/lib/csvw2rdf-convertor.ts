...
export class CSVW2RDFConvertor {
    ...
    public constructor(config?: unknown, pathOverrides?: Record<string, string>, offline?: boolean) {
        this.config = config;
        this.pathOverrides = pathOverrides;
        this.offline = offline;
    }

    public async convert(input?: string, output?: string, format?: RDFSerialization) {
    ...
    private getUri(prefix: string): Promise<string | null> {
        return fetch(`https://prefix.cc/${prefix}.file.json`)
    ...
    private getPrefix(uri: string): Promise<string | null> {
        return fetch(`https://prefix.cc/reverse?uri=${uri}&format=json`)
            .then(response => response.json() as Promise<PrefixCCResponse>)
