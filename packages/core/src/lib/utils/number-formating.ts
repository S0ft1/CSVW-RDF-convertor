...
function findFormatedColumns(allColumns: CsvwColumnDescription[]) {
    ...
    for (const column of allColumns) {
      if (column.datatype) {
        let dataType = column.datatype as CsvwDatatype;
        if (dataType.format) {
  ...
  function transformNumber(value: string, columnDescription: ColumnDescriptionWithDataTypeAndFormat,issueTracker: IssueTracker): string {
    if (columnDescription.datatype.format.pattern) {
      if (!patternIsValid(columnDescription.datatype.format.pattern)) {
        issueTracker.addError(`Invalid pattern: ${columnDescription.datatype.format.pattern} take a look at https://www.unicode.org/reports/tr35/tr35-39/tr35-numbers.html#Number_Patterns`, true);
      }
      ...
      if (splitted.length == 1) {
        ...
        let integerPattern = splittedPattern[0];
        let decimalPattern = splittedPattern[1];
        let reversedIntegerPart = integerPart.split('').reverse().join('');
        let reversedIntegerPattern = integerPattern.split('').reverse().join('');
        return transformNumberInner(reversedIntegerPattern, reversedIntegerPart, groupChar).split('').reverse().join('') +
          decimalChar +
          transformNumberInner(decimalPattern,decimalPart, groupChar);
      }
      else {
        issueTracker.addError(`Too many decimal characters in value: ${value}`, true);
        return value;
      }
    }
    return value;
  ...
  function transformNumberInner(pattern: string, number: string, groupChar: string): string {
    let transformedPart = "";
 ...
 */
 export function transformNumbersInTable(jsonTable: string, descriptor: DescriptorWrapper, issueTracker: IssueTracker): string {
    ...
    let colums = descriptor.descriptor.tableSchema?.columns;
    if (colums) {
      let formatedColumns = findFormatedColumns(colums);
      for (const column of formatedColumns) {
        if (column.name) {
          const regex = new RegExp(`"${column.name}"${rgxForNumberRowInJson}`, 'g');
