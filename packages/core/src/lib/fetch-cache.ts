...
export interface FetchCacheInterface {
    ...
    toCache(url: string, baseIri: string, content: Promise<string>): Promise<void>;
}
...
export class DefaultFetchCache{
    ...
    public async toCache(url: string, baseIri: string, content: Promise<string>): Promise<void> {
